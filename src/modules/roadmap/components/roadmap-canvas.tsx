...
import { Filter, Eye, Award, GraduationCap, Pencil } from "lucide-react";
  ...
  return (
    <div className="relative w-full h-[calc(100vh-64px)] bg-surface-canvas overflow-hidden select-none">
      ...
      <div className="absolute top-4 left-4 z-10 flex flex-wrap items-center gap-3 p-3.5 rounded-lg bg-surface-card/90 border border-border-subtle backdrop-blur-xl shadow-2xl">
        ...
        {/* Author Studio Link */}
        <Link href={ROUTES.STUDIO(initialData.course.slug)}>
          <Button
            variant="secondary"
            size="sm"
            leftIcon={<Pencil className="w-3.5 h-3.5 text-status-diff" />}
            title="Открыть Авторскую Студию (импорт тем, редактирование графа)"
          >
            Студия
          </Button>
        </Link>
