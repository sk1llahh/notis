# NOTIS — Системный паспорт и руководство для ИИ-агентов (AI Agent Context & System Passport)
...
Данный документ представляет собой исчерпывающий системный паспорт проекта **Notis**. Он предназначен для того, чтобы любой новый ИИ-агент (Claude, Gemini, Cursor Agent, Windsurf и др.) мог мгновенно получить 100% контекста о кодовой базе, архитектурных инвариантах, доменных движках, базе данных и правилах безопасной модификации кода **без доступа к истории предыдущих чатов**.
...
2. `pnpm test` (`tsx --test 'src/**/*.test.ts'`) — запуск всех 290 юнит- и интеграционных тестов.
...
> В репозитории зафиксировано **290 активных тестов**. Ни один тест не должен быть сломан, удален или закомментирован. Любая новая функциональность или Server Action обязаны сопровождаться модульными тестами, покрывающими валидные и невалидные сценарии.
...
| **`tsx`** | `^4.23.13` | Рантайм для быстрого запуска сид-скрипта (`prisma/seed.ts`) и нативных тестов Node.js. |
...
> Запрещается реализовывать логику типов вопросов через разрозненные ветки `switch(type)` или `if(type === ...)` в сервисах оценки и компонентах UI.
...
### 4.3. Изолированная песочница кода (Web Worker Sandbox)
...
6. **Серверный / SSR Fallback**:
   - Для выполнения тестов в среде Node.js (при серверной проверке квиза и в модульных тестах) используется `executeCodeNodeFallback()` на базе модуля `node:vm` с изолированным контекстом и таймаутом.
...
4. **Темы и граф роадмапа**:
   ...
   - `TopicTranslation`: заголовок `title`, выжимка `summary`, описание `description`, списки `keyPoints` (String[]), `pitfalls` (String[]), блоки `contentBlocks` (Json).
...
### 5.2. Каталог «СПЯЩИХ» (DORMANT) моделей в схеме

В `prisma/schema.prisma` присутствуют модели, заложенные на этапе проектирования под расширения функционала (PRD Backlog P1/P2).  
Они **не имеют активного UI-интерфейса в текущей версии**, но **строго интегрированы в схему реляционных связей**:

> [!WARNING]
> **НЕ УДАЛЯТЬ И НЕ МОДИФИЦИРОВАТЬ ЭТИ МОДЕЛИ БЕЗ СОГЛАСОВАНИЯ**:  
> Удаление спящих моделей сломает типы Prisma Client, нарушит работу сид-скрипта `prisma/seed.ts` и транзакционного удаления аккаунтов `deleteAccountAction`.

#### 1. Подсистема биллинга и монетизации (PRD P2 Backlog):
- **`Plan`**: тарифные планы платформы (`type: PlanType (ALL_ACCESS, SPECIFIC_COURSES)`, `interval: BillingInterval (MONTHLY, YEARLY, LIFETIME)`, `priceMinor`, `currency`).
- **`PlanTranslation`**: локализованные названия и описания планов.
- **`PlanCourse`**: связка тарифа со списком доступных курсов.
- **`Subscription`**: подписки пользователей через платежные шлюзы (Stripe/CloudPayments) с полями `currentPeriodEnd`, `cancelAtPeriodEnd`, `providerRef`.
- **`Payment`**: история транзакций оплаты курсов и подписок (`amountMinor`, `refundedMinor`, `status: PaymentStatus`).

#### 2. Подсистема тегов и каталогизации:
- **`Tag`**: справочник технологических тегов (`slug`).
- **`TagTranslation`**: переводы названий тегов.
- **`CourseTag`**: связь многие-ко-многим между курсами и тегами (`isPrimary`, `order`).
- **`TopicTag`**: связка тем с тегами для контекстной фильтрации.
...
## 7. Чек-лист для ИИ-агента перед сдачей работы
...
- [ ] Типы вопросов квизов не хардкодятся через `switch/case`, а используют реестр `QuestionStrategyRegistry`.
...
- [ ] Любое удаление связей графа или сущностей защищено транзакцией `prisma.$transaction` с пошаговой зачисткой зависимостей во избежание `P2003`.
- [ ] Добавление ребер в роадмап проверяется через DFS Cycle Guard `wouldCreateCycle`.
- [ ] Новая логика покрыта модульными тестами (`src/**/*.test.ts`).
